...
class Book {
  ...
  getState() {
    let getOrdersFromBook = (ordersByPrice = {}) => {
      return Object.values(ordersByPrice).reduce((cur, node) => {
        return cur.concat(node.orders);
      }, []);
    };
  ...
  matchOrder(match) {
    ...
    let price = num(match.price).set_precision(2);
    let hash = this._getHash(match.side);
    let node = hash[price.toString()];
    ...
    this._ordersById[order.id] = order;
    if (order.size.eq(0)) this.removeOrder(order.id);
  }
  changeOrder(change) {
